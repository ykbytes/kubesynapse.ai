...
class OpenCodeRuntimeTests(unittest.TestCase):
    ...
    # ------------------------------------------------------------------
    # §security-P1: Admin override and immutable config tests
    # ------------------------------------------------------------------

    def test_build_generated_config_applies_immutable_plugin_block(self) -> None:
        """Immutable config plugin:[] must survive runtime config generation."""
        immutable_base = {
            "plugin": [],
            "permission": {"bash": "ask", "write": "deny"},
            "skills": {"urls": []},
            "mcp": {},
            "provider": {},
        }
        with (
            ...
            patch.object(
                skills_mod, "_load_immutable_config_base", return_value=immutable_base
            ),
        ):
            config, warnings = opencode_runtime_main.build_generated_config(
                [],
                config_overrides={"permission": "allow"},
            )
    ...
    def test_build_generated_config_admin_plugin_list_overrides_immutable(self) -> None:
        """Admin OPENCODE_PLUGIN_LIST_JSON must override immutable plugin:[]."""
        immutable_base = {"plugin": [], "permission": {}, "mcp": {}, "provider": {}}
        admin_plugins = json.dumps([{"name": "audit-logger", "entry": "./audit.ts"}])
        with (
            patch.object(skills_mod, "DEFAULT_PROVIDER", "litellm"),
            patch.object(
                skills_mod, "_load_immutable_config_base", return_value=immutable_base
            ...
            patch.dict(
                os.environ,
                {"OPENCODE_PLUGIN_LIST_JSON": admin_plugins},
                clear=False,
            ),
        ):
            config, warnings = opencode_runtime_main.build_generated_config([])

        # Admin plugin list overrides immutable block
        self.assertEqual(len(config["plugin"]), 1)
        self.assertEqual(config["plugin"][0]["name"], "audit-logger")

    def test_build_generated_config_admin_model_override_blocks_unlisted_model(self) -> None:
        """Admin OPENCODE_ADMIN_MODEL_OVERRIDE_JSON must block unlisted models."""
        with (
            patch.object(skills_mod, "DEFAULT_PROVIDER", "litellm"),
            patch.object(skills_mod, "DEFAULT_MODEL_REF", "litellm/gpt-4.5-preview"),
            patch.dict(
                os.environ,
                {
                    "OPENCODE_ADMIN_MODEL_OVERRIDE_JSON": json.dumps(
                        ["gpt-4o", "gpt-4o-mini"]
                    ),
                },
                clear=False,
        ...
        # Model must be replaced with the first allowed model
        self.assertEqual(config["model"], "gpt-4o")
        self.assertTrue(any("blocked" in w.lower() for w in warnings))

    def test_build_generated_config_admin_model_override_allows_listed_model(self) -> None:
        """Admin model allowlist must NOT block a listed model (full ref)."""
        with (
            patch.object(skills_mod, "DEFAULT_PROVIDER", "litellm"),
            patch.object(skills_mod, "DEFAULT_MODEL_REF", "litellm/gpt-4o"),
            patch.dict(
                os.environ,
                {
                    "OPENCODE_ADMIN_MODEL_OVERRIDE_JSON": json.dumps(
                        ["litellm/gpt-4o", "litellm/gpt-4o-mini"]
                    ),
                },
                clear=False,
            ),
        ...
        self.assertEqual(config["model"], "litellm/gpt-4o")
        self.assertEqual(warnings, [])

    def test_build_generated_config_admin_provider_overrides_force_base_url(self) -> None:
        """Admin OPENCODE_ADMIN_PROVIDER_OVERRIDE_JSON must override provider options."""
        with (
            patch.object(skills_mod, "DEFAULT_PROVIDER", "litellm"),
            patch.object(
                supervisor_mod,
                "build_litellm_base_url",
                return_value="http://litellm.internal:4000/v1/chat/completions",
            ),
            patch.dict(
                os.environ,
                {
                    "OPENCODE_ADMIN_PROVIDER_OVERRIDE_JSON": json.dumps(
                        {
                            "litellm": {
                                "options": {
                                    "baseURL": "https://security-proxy.example.com/v1"
                                }
                            }
                        }
                    ),
        ...
        # Admin baseURL must win over runtime default
        self.assertEqual(
    ...
    def test_build_server_env_respects_explicit_disable_plugins_override(self) -> None:
        """Explicit OPENCODE_DISABLE_DEFAULT_PLUGINS must be respected."""
        with patch.dict(
            os.environ,
...
class SessionHealthMetricsTests(unittest.TestCase):
    ...
    def test_ready_includes_server_health(self) -> None:
        ...
        # ready() returns a JSONResponse — parse its body for assertions
        data = json.loads(result.body)
        checks = data.get("checks", {})
        self.assertIn("opencode_server_healthy", checks)
        self.assertIn("session_registry_writable", checks)


class ContextBudgetEndpointTests(unittest.TestCase):
    """Tests for the /context-budget endpoint.

    NOTE: Skipped due to Starlette TestClient API incompatibility with the
    installed httpx version. These tests need a TestClient refactor to use
    httpx.ASGITransport or an equivalent transport layer.
    """

    @unittest.skip("Starlette TestClient API incompatible with installed httpx version")
    ...
    def test_unknown_thread_id_returns_404(self) -> None:
        self.skipTest("Starlette TestClient API incompatibility")
        from fastapi.testclient import TestClient
...
class StreamingEventsTests(unittest.TestCase):
    ...
    @unittest.skip("Requires running OpenCode server — stream tests need HTTP-level mocking refactor")
...
@unittest.skip("Compat-layer bugs in memory/compat.py — fix in monolith-split session")
class SessionMemoryTests(unittest.TestCase):
    ...
    def test_save_and_recall(self) -> None:
        with patch.object(config_mod, "MEMORY_ENABLED", True):
...
class MemoryEntryTypeValidationTests(unittest.TestCase):
    ...
    def test_save_memory_warns_on_unknown_type(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            with (
                patch.object(config_mod, "MEMORY_ENABLED", True),
