"""Backward-compatible memory exports for the OpenCode runtime.

This module provides the legacy API (SessionMemory, SESSION_MEMORY, etc.)
that the rest of the opencode-runtime codebase expects, while delegating
to the new multi-tier memory system underneath.
"""
...
def _legacy_to_entry(thread_id: str, entry: dict[str, Any]) -> MemoryEntry:
    ...
    with contextlib.suppress(ValueError):
        retention_str = entry.get("retention", "SESSION")
...
class SessionMemory:
    ...
    def __init__(self, base_dir: Path, *, max_thread: int = 100, max_workspace: int = 50):
        ...
        builtin = BuiltinMemoryProvider(
            base_dir=base_dir,
            max_session_entries=max_thread,
            max_workspace_entries=max_workspace,
        )
        self._manager.add_provider(builtin)

        # Semantic memory is optional — try to add it if Qdrant is reachable
        try:
            from config import (
            ...
            logger.warning("Failed to initialize semantic memory provider", exc_info=True)
...
# ---------------------------------------------------------------------------
# Module-level singleton (lazy initialization to avoid import-time side effects)
# ---------------------------------------------------------------------------
