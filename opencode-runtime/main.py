...
# §observability-P1: Logging filter that auto-injects request_id from
# the async context into every log record. Works with uvicorn, FastAPI,
# and the OpenCode subprocess supervisor — any code that logs while
# a request_id is set in _REQUEST_ID_CTX gets it for free.
class _RequestIdLogFilter(logging.Filter):
    """Inject the current request_id from _REQUEST_ID_CTX into log records."""
...
_root.addFilter(_RequestIdLogFilter())
...
def _emit_llm_call_from_response(
    *,
    ...
    reasoning_text = (metadata or {}).get("reasoning_text", "")
