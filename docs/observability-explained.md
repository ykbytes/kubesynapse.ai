...
| `llm_call_completed` | After each LLM interaction | model, provider, tokens (prompt/completion/cache_read/cache_write/reasoning), cost, latency, preview |
| `tool_call_completed` / `tool_call_failed` | After each tool use | tool name, args (`tool_args`, `tool_result` — JSON objects), error, `duration_ms`, `started_at`. Tool output is capped at 40,000 characters to keep trace storage manageable while preserving meaningful result data. `duration_ms` is natively extracted from OpenCode's per-tool `state.time.start`/`state.time.end` timestamps. |
...
LLM call records include a full token breakdown beyond simple prompt/completion counts:

| Field | Description |
|---|---|
| `prompt_tokens` | Input tokens sent to the model |
| `completion_tokens` | Output tokens generated by the model |
| `cache_read_tokens` | Tokens served from provider prompt cache (reduces cost) |
| `cache_write_tokens` | Tokens written into prompt cache for future reuse |
| `reasoning_tokens` | Tokens consumed by chain-of-thought reasoning (o-series, etc.) |
| `total_tokens` | Aggregate of all token categories |

The runtime reads these from `metadata.tokens.input/output/cache_read/cache_write/reasoning` in the OpenCode response. The operator worker and direct-invoke gateway handler both propagate the full breakdown into `LLMCallRecord` and `WorkflowExecution` rows.

The Observatory UI displays:
- **Token Breakdown panel**: stacked bar showing prompt vs completion vs reasoning with cache-read overlay
- **Cache Hit Ratio**: percentage of prompt tokens served from cache (`cache_read / prompt_tokens`)
- **Quality Flags**: warns when token data is missing (0 total tokens despite LLM calls)

### Tool Call Duration

Each tool call record carries a `duration_ms` field representing wall-clock time spent executing the tool. For the OpenCode runtime, this is natively extracted from the per-tool `state.time` object:

```json
{
  "state": {
    "time": { "start": 1780340516613, "end": 1780340516628 },
    "status": "completed"
  }
}
```

Duration is computed as `end - start` (millisecond timestamps). The Observatory's **Tool Mix** chart uses this to show which tools consumed the most wall-clock time.

### Observatory UI Features

The Observatory workspace provides rich inspection of execution traces:
...
| **ResultBlock** | Smart result preview with truncation, JSON auto-detection, diff detection, and expand/collapse toggle |
...
| `reasoning_tokens` | `INTEGER` | Tokens used for chain-of-thought reasoning |
...
Runtime event streams are complementary to execution traces. They do not have to carry the
same full `tool_result` payloads rendered in the Observatory. Those durable tool records are
constructed from the runtime's final response payload and forwarded by the operator.
...
- **Overview tab**: Key metrics (steps, LLM calls, tool calls, tokens, cost) with signal warnings, plus run-level insight charts:
  - *Recent Run Trend* — duration sparkline across the last runs of the same workflow, color-toned by phase (success, running, failed).
  - *Step Contribution* — horizontal share bars showing which steps dominate total runtime, useful for spotting a runaway step.
  - *Step Variability* — min/median/max range bars per step with a current-run marker, surfacing steps whose latency is high relative to history.
  - *Tool Mix* — which MCP tools consumed the most wall-clock time, weighted by latency with failure counts in the tooltip.
  - *Model Efficiency* — token-vs-latency scatter for every LLM call, bubble size by cost.
...
### Execution shows "0 tokens" despite LLM calls being recorded

**Cause:** The operator worker and gateway were reading token counts from the wrong path in the runtime response. The runtime returns `metadata.tokens.input/output/reasoning/cache_read/cache_write` (nested under `tokens`) but the worker was reading flat `metadata.prompt_tokens`.

**Fix:** Both the operator worker (`_emit_traces_from_result`, `summarize_execution_metrics`) and the gateway direct-invoke handler now read from `metadata.tokens.*`. The runtime's `_sync_emit` also includes `cache_read_tokens`, `cache_write_tokens`, and `reasoning_tokens` in event envelopes.
...
### Tool results render as raw text instead of prettified JSON

**Cause:** Older runs may contain runtime-truncated `tool_result` payloads that ended mid-object,
or the run was captured before the 40,000-character extraction fix landed.

**Fix:** Fresh runs on the updated OpenCode runtime preserve much larger tool payloads, and the
Web UI now auto-repairs common truncation cases by closing unbalanced braces before attempting
