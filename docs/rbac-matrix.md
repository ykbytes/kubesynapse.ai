...
**Component**: FastAPI Gateway (REST API + SSE Streaming)
...
| API Group | Resource | Verbs | Justification |
|-----------|----------|-------|---------------|
| `""` (core) | `secrets` | get, list, create, update, delete | Manage provider credentials and stored tokens in the gateway namespace |
...
### ClusterRole (cross-namespace additions)
In addition to the CRD access above, the gateway ClusterRole also grants:
| `""` (core) | `secrets` | get, list, watch, create, update, delete | Cross-namespace: agent git-credentials, github-credentials, provider bootstrap secrets |
| `""` (core) | `pods/log` | get | Cross-namespace: serve /agents/{name}/logs for agents in any namespace |

### Justification Summary
- **Why ClusterRole?** The gateway needs cluster-wide CRD read/write access to serve the REST API. It also creates or updates `AgentTenant` CRs when admins create or modify local users.
- **Why cross-namespace secrets?** The gateway manages agent credential secrets (git-credentials, github-credentials, provider bootstrap) in agent namespaces.
- **Why cross-namespace pods/log?** The `/agents/{name}/logs` endpoint reads pod logs from agent namespaces.
- **What was removed:** `pods/exec` verb was removed (artifact reads use the sidecar pattern instead).
- **What it CANNOT do:** Create/delete pods, services, statefulsets, namespaces, or RBAC bindings directly.
