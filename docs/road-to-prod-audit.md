...
> **Scope**: Full-depth audit of the `kubemininions` agent orchestration platform — operator, runtimes, API gateway, Helm chart, and supporting infrastructure — with actionable rewrites and standards alignment for production-grade credibility.
...
**Problem**: A single `ai-agent-sandbox` chart deploys the entire platform: PostgreSQL, Redis, Qdrant, NATS, LiteLLM, Operator, API Gateway, Web UI, CRDs, RBAC, NetworkPolicies. This is ~26 templates in one chart.
