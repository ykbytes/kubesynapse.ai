...
def _configure_logging() -> None:
    """Configure root logger with JSON-formatted structured logging.

    Uses python-json-logger when available, falls back to plain text.
    The structured_logging module's StructuredFormatter can be enabled
    via STRuctURED_LOGGING=true for richer field extraction.
    """
    ...
    if os.getenv("JSON_LOGS", "true").lower() in {"1", "true"} and _jsonlogger is not None:
        handler.setFormatter(_jsonlogger.JsonFormatter(
    ...
    from structured_logging import _request_id_filter
...
async def lifespan(app: FastAPI):
    # Enable structured JSON logging when STRuctURED_LOGGING env is set
    if os.getenv("STRUCTURED_LOGGING", "").strip().lower() in ("1", "true", "yes"):
...
# §observability-P1: Request ID middleware — propagates x-request-id through
# all log entries and downstream HTTP calls for end-to-end tracing.
@app.middleware("http")
async def _request_id_middleware(request: Request, call_next):  # type: ignore[no-untyped-def]
    from structured_logging import _REQUEST_ID_CTX, get_request_id
...
# §error-handling-P1: Convert all exceptions to standardised ErrorResponse format
# so the UI and CLI can programmatically determine what went wrong.
...
async def _http_exception_handler(request: Request, exc: HTTPException) -> JSONResponse:
    ...
    # If the detail is already an ErrorResponse dict, use it directly
...
async def _validation_exception_handler(
    ...
    """Convert FastAPI validation errors to standard ErrorResponse."""
    from error_codes import ErrorCode, build_error_response

    errors = exc.errors()
    detail_parts: list[str] = []
    for err in errors[:5]:  # Cap at 5 to avoid huge responses
...
async def _generic_exception_handler(request: Request, exc: Exception) -> JSONResponse:
    """Catch-all for unhandled exceptions — always return structured errors."""
    from error_codes import ErrorCode, build_error_response
...
class AgentDetail(AgentInfo):
    ...
    created_at: str | None = None
    reconcile_error: str | None = Field(default=None, description="Reconciliation error from the operator")
...
class WorkflowTriggerRequest(BaseModel):
    ...
    def normalize_legacy_fields(cls, data: Any) -> Any:
        if not isinstance(data, dict):
            return data
        payload = dict(data)
        if not payload.get("source_ref") and payload.get("source_name"):
            payload["source_ref"] = payload["source_name"]
        workflow_ref = payload.get("workflow_ref")
        if not workflow_ref:
            workflow_name = payload.get("target_workflow_name")
            workflow_namespace = payload.get("target_workflow_namespace")
            if workflow_name is not None or workflow_namespace is not None:
...
class WorkflowTriggerUpdateRequest(BaseModel):
    ...
    def normalize_legacy_fields(cls, data: Any) -> Any:
        if not isinstance(data, dict):
            return data
        payload = dict(data)
        if payload.get("source_ref") is None and payload.get("source_name") is not None:
            payload["source_ref"] = payload["source_name"]
        workflow_ref = payload.get("workflow_ref")
...
def _extract_reconcile_error(agent: dict[str, Any]) -> str | None:
    """Extract reconciliation error from CRD status conditions."""
    ...
    for condition in conditions:
        ...
        if "error" in cond_type or "failed" in cond_type:
            return str(condition.get("message") or condition.get("reason") or "Reconciliation failed")
    # Fallback: check for generic error in status
    if isinstance(status.get("error"), str) and status["error"].strip():
...
def agent_detail_from_resource(agent: dict[str, Any]) -> AgentDetail:
    ...
    return AgentDetail(
        **info.model_dump(),
        ...
        reconcile_error=_extract_reconcile_error(agent),
    )
...
def build_agent_collaboration_system_note(agent_name: str, namespace: str, agent: dict[str, Any]) -> str | None:
    ...
    if not runtime_supports_direct_chat_delegation(runtime_kind):
        if runtime_kind == "opencode":
            ...
            lines.append(
                "- To actually query a peer from standard chat, use the bash tool to POST JSON-RPC 2.0 to"
                f" $API_GATEWAY_INTERNAL_URL/a2a/<agent>?namespace=<namespace> with the agent's session"
                " bearer token (available as API_GATEWAY_TOKEN in the runtime environment)"
                f" and A2A-Version: {A2A_PROTOCOL_VERSION}."
            )
            lines.append(
