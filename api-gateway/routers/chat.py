...
# ── Chat threads / history compatibility (deprecated alias for CLI) ──


@router.get("/chat/threads")
async def api_list_chat_threads(
    agent_name: str | None = None,
    namespace: str = "default",
    limit: int = 20,
    user=Depends(verify_token),
):
    """Deprecated alias for GET /chat-sessions.  Supported for CLI backward compatibility."""
    ensure_namespace_access(user, namespace)
    username = user.get("sub") or user.get("username")

    sessions = list_chat_sessions(namespace, agent_name or "", username=username)
    if agent_name:
        sessions = [s for s in sessions if s.get("agent_name") == agent_name]
    sessions = sessions[:limit]

    # Adapt to the legacy thread format expected by the CLI
    return [
        {
            "thread_id": s.get("session_id"),
            "agent_name": s.get("agent_name"),
            "message_count": len(s.get("summary", "")),
...
async def api_get_chat_thread_history(
    thread_id: str,
    limit: int = 50,
    user=Depends(verify_token),
):
    """Deprecated alias for GET /chat-sessions/{id}/messages.  Supported for CLI backward compatibility."""
    _validate_session_ownership(thread_id, user)
    messages = get_chat_session_messages(thread_id)[:limit]
...
async def _fetch_opencode_zen_models(api_key: str) -> list[dict[str, str]]:
    try:
        ...
        logger.warning("OpenCode Zen models endpoint returned HTTP %s", exc.response.status_code)
        raise RuntimeError(f"OpenCode API returned HTTP {exc.response.status_code}") from exc
    except Exception as exc:
        logger.warning("Failed to fetch OpenCode Zen models: %s", exc)
        raise RuntimeError(f"Failed to reach OpenCode API: {exc}") from exc
...
async def _fetch_opencode_go_models(api_key: str) -> list[dict[str, str]]:
    try:
        ...
        logger.warning("OpenCode Go models endpoint returned HTTP %s", exc.response.status_code)
        raise RuntimeError(f"OpenCode API returned HTTP {exc.response.status_code}") from exc
    except Exception as exc:
