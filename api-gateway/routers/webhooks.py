...
def _trigger_workflow_ref(
    ...
    return workflow_name, workflow_namespace
...
def create_trigger(body: WorkflowTriggerRequest, namespace: str = "default", user=Depends(verify_token)):
    ...
    workflow_name, workflow_namespace = _trigger_workflow_ref(body, namespace)
    try:
        row = create_workflow_trigger(
            ...
            target_workflow_name=workflow_name or "",
            target_workflow_namespace=workflow_namespace or namespace,
...
def update_trigger(name: str, body: WorkflowTriggerUpdateRequest, namespace: str = "default", user=Depends(verify_token)):
    ...
    if existing is None:
        raise HTTPException(status_code=404, detail="Workflow trigger not found")
    source_ref = _trigger_source_ref(body)
    target_workflow_name, target_workflow_namespace = _trigger_workflow_ref(body, namespace)
    if getattr(body, "workflow_ref", None) is None and getattr(body, "target_workflow_name", None) is None:
