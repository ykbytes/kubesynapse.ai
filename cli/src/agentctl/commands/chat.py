...
def chat_send(
        ...
        try:
            with _api() as client:
                with client.stream(
                    ...
                    console.print(
                        Panel(
                            Text(safe_text(agent_name), style="bold"),
            ...
            console.print(f"\n[dim]{stop_marker()} Chat interrupted[/dim]")
        return

    # Non-streaming
    try:
        with console.status(f"[bold cyan]Sending to {agent_name}...[/bold cyan]", spinner="dots2"):
            with _api() as client:
    ...
    if response_text.strip():
        renderable = Markdown(response_text) if supports_unicode_output() else Text(safe_text(response_text))
        console.print(Panel(renderable, border_style="bright_cyan", box=preferred_box()))
    else:
...
def chat_threads(
    ...
    try:
        with console.status("[bold cyan]Loading threads...[/bold cyan]", spinner="dots2"):
            with _api() as client:
...
def chat_history(
    ...
    try:
        with console.status("[bold cyan]Loading history...[/bold cyan]", spinner="dots2"):
            with _api() as client:
    ...
    for msg in messages:
        ...
        renderable = Markdown(content) if role == "assistant" and supports_unicode_output() else Text(safe_text(content))
...
def chat_interactive(
    ...
    console.print(
        Panel(
            Text.assemble(
                ...
                (
                    "\nType your message and press Enter. Type /exit, /quit, or press Ctrl+C to end.",
                    "dim",
                ),
            ),
    ...
    try:
        while True:
            try:
                user_input = console.input(f"[bold bright_cyan]{prompt_prefix('you')}[/bold bright_cyan] ").strip()
            except EOFError:
                console.print()
                break
            ...
            try:
                with _api() as client:
                    with client.stream(
                        ...
                        console.print(Text(prompt_prefix("agent"), style="bold green"), end="")
                        collected = ""
                        for sse in client.iter_sse(response):
                            event = sse["event"]
                            data_str = sse["data"]
                            if event in ("chat.delta", "response.delta"):
                                ...
                                console.print(
                                    Text(
                                        f"\n{error_marker()} Error: {safe_text(event_data.get('error', 'Unknown'))}",
                                        style="bold red",
                                    )
                                )
                                break
                ...
                console.print(Text(f"\n{error_marker()} {safe_text(exc)}", style="bold red"))

    except KeyboardInterrupt:
        pass

    console.print()
    divider = Panel("Session ended", border_style="dim", box=preferred_box(simple=True))
