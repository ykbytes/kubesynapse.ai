...
def credentials_git_set(
    ...
    try:
        with console.status(f"[bold cyan]Setting git credentials for {agent_name}...[/bold cyan]"):
            with _api() as client:
                data = client.post(
...
def credentials_git_show(
    ...
    settings = get_settings()
    try:
        with console.status(f"[bold cyan]Loading git credentials for {agent_name}...[/bold cyan]"):
            with _api() as client:
                data = client.get(
                    f"/api/agents/{agent_name}/git-credentials",
                    params=_ns_params(),
...
def credentials_git_delete(
    ...
    try:
        with console.status(f"[bold cyan]Deleting git credentials for {agent_name}...[/bold cyan]"):
            with _api() as client:
...
def credentials_github_set(
    ...
    try:
        with console.status(f"[bold cyan]Setting GitHub credentials for {agent_name}...[/bold cyan]"):
            with _api() as client:
                data = client.post(
...
def credentials_github_show(
    ...
    settings = get_settings()
    try:
        with console.status(f"[bold cyan]Loading GitHub credentials for {agent_name}...[/bold cyan]"):
            with _api() as client:
                data = client.get(
                    f"/api/agents/{agent_name}/github-credentials",
                    params=_ns_params(),
...
def credentials_github_delete(
    ...
    try:
        with console.status(f"[bold cyan]Deleting GitHub credentials for {agent_name}...[/bold cyan]"):
            with _api() as client:
