...
def auth_login(
    ...
    try:
        with console.status("[bold cyan]Logging in...[/bold cyan]", spinner="dots2"):
            with _api() as client:
    ...
    role = str(user.get("role") or data.get("role", "-"))
    display_user = str(user.get("username") or data.get("username", username))
...
def auth_logout(
    ...
    if revoke:
        try:
            with console.status("[bold cyan]Logging out...[/bold cyan]", spinner="dots2"):
                with _api() as client:
...
def auth_register(
    ...
    try:
        with console.status("[bold cyan]Registering...[/bold cyan]", spinner="dots2"):
            with _api() as client:
...
def auth_me() -> None:
    ...
    settings = get_settings()
    try:
        with console.status("[bold cyan]Loading user context...[/bold cyan]", spinner="dots2"):
            with _api() as client:
...
def auth_change_password(
    ...
    try:
        with console.status("[bold cyan]Updating password...[/bold cyan]", spinner="dots2"):
            with _api() as client:
                client.post(
...
def auth_config() -> None:
    ...
    settings = get_settings()
    try:
        with console.status("[bold cyan]Loading auth config...[/bold cyan]", spinner="dots2"):
            with _api() as client:
