...
def coerce_policy_payload(document: dict[str, Any], *, for_update: bool) -> tuple[dict[str, Any], str | None]:
    """Parse a policy document into a gateway API payload."""
    metadata = document.get("metadata") if isinstance(document.get("metadata"), dict) else {}

    def _guardrails(value: Any) -> dict[str, Any]:
        if not isinstance(value, dict):
            return {}
        return dict(value)

    if document.get("kind") == "AgentPolicy" and isinstance(document.get("spec"), dict):
        spec = document["spec"]
        payload: dict[str, Any] = {
            "input_guardrails": _guardrails(spec.get("inputGuardrails")),
            "output_guardrails": _guardrails(spec.get("outputGuardrails")),
            "allowed_models": normalize_list_of_strings(spec.get("allowedModels"), "allowedModels"),
            "allowed_mcp_servers": normalize_list_of_strings(spec.get("allowedMcpServers"), "allowedMcpServers"),
            "mcp_require_hitl": bool(spec.get("mcpRequireHitl", True)),
            "tool_policy": _guardrails(spec.get("toolPolicy")),
            "memory_policy": _guardrails(spec.get("memoryPolicy")),
        }
        if not for_update:
            payload["name"] = str(metadata.get("name", ""))
        return payload, str(metadata.get("name", "") or "")
    ...
    payload = {
        "input_guardrails": _guardrails(snake_or_camel(document, "input_guardrails", "inputGuardrails", {})),
        "output_guardrails": _guardrails(snake_or_camel(document, "output_guardrails", "outputGuardrails", {})),
        "allowed_models": normalize_list_of_strings(
            snake_or_camel(document, "allowed_models", "allowedModels", []), "allowed_models"
        ),
        "allowed_mcp_servers": normalize_list_of_strings(
            snake_or_camel(document, "allowed_mcp_servers", "allowedMcpServers", []), "allowed_mcp_servers"
        ),
        "mcp_require_hitl": bool(snake_or_camel(document, "mcp_require_hitl", "mcpRequireHitl", True)),
        "tool_policy": _guardrails(snake_or_camel(document, "tool_policy", "toolPolicy", {})),
        "memory_policy": _guardrails(snake_or_camel(document, "memory_policy", "memoryPolicy", {})),
    }
    resource_name = str(snake_or_camel(document, "name", "name", metadata.get("name", "")) or "")
    if not for_update:
        payload["name"] = resource_name
