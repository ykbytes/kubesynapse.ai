...
def skills_list(
    ...
    try:
        with console.status("[bold cyan]Loading skills catalog...[/bold cyan]", spinner="dots2"):
            with _api() as client:
...
def skills_show(
    ...
    settings = get_settings()
    try:
        with console.status(f"[bold cyan]Loading skill {skill_id}...[/bold cyan]", spinner="dots2"):
            with _api() as client:
...
def tools_list() -> None:
    ...
    settings = get_settings()
    try:
        with console.status("[bold cyan]Loading tools...[/bold cyan]", spinner="dots2"):
            with _api() as client:
...
def tools_hub() -> None:
    ...
    settings = get_settings()
    try:
        with console.status("[bold cyan]Loading MCP hub...[/bold cyan]", spinner="dots2"):
            with _api() as client:
