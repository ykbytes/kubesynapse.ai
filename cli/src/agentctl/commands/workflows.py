...
def workflows_list() -> None:
    ...
    try:
        with console.status("[bold cyan]Loading workflows...[/bold cyan]"):
            with _api() as client:
...
def workflows_show(workflow_name: str = typer.Argument(..., help="Workflow name.")) -> None:
    ...
    settings = get_settings()
    try:
        with console.status(f"[bold cyan]Loading workflow {workflow_name}...[/bold cyan]"):
            with _api() as client:
...
def workflows_delete(
    ...
    try:
        with console.status(f"[bold cyan]Deleting workflow {resolved_name}...[/bold cyan]"):
            with _api() as client:
...
def workflows_trigger(
    ...
    try:
        with console.status(f"[bold cyan]Triggering workflow {workflow_name}...[/bold cyan]"):
            with _api() as client:
...
def workflows_status(workflow_name: str = typer.Argument(..., help="Workflow name.")) -> None:
    ...
    settings = get_settings()
    try:
        with console.status(f"[bold cyan]Loading status for {workflow_name}...[/bold cyan]"):
            with _api() as client:
                data = client.get(f"/api/workflows/{workflow_name}", params=_ns_params())
    except ApiError as exc:
        fatal(str(exc))
    ...
    if isinstance(step_states, dict) and step_states:
        st = Table(title="Step States", border_style="bright_black")
...
def workflows_logs(
    ...
    try:
        with console.status(f"[bold cyan]Fetching logs for {workflow_name}...[/bold cyan]"):
            with _api() as client:
