...
def artifacts_list(
    ...
    try:
        with console.status("[bold cyan]Loading artifacts...[/bold cyan]", spinner="dots2"):
            with _api() as client:
...
def artifacts_show(
    ...
    settings = get_settings()
    try:
        with console.status(f"[bold cyan]Loading artifact {artifact_id[:16]}...[/bold cyan]", spinner="dots2"):
            with _api() as client:
...
def artifacts_download(
    ...
    settings = get_settings()
    try:
        with console.status(f"[bold cyan]Downloading artifact {artifact_id[:16]}...[/bold cyan]", spinner="dots2"):
            with _api() as client:
