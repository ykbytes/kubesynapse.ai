...
class ApiClient:
    ...
    def _raise_for_status(response: httpx.Response) -> None:
        ...
        detail = None
        # On error paths, read body safely before accessing .text / .json().
        # Streaming responses (from client.stream) need read() before decode.
        try:
            response.read()
        except Exception:
            pass
        try:
            ...
            message = body_text.strip() or response.reason_phrase or "Unknown error"
        except Exception:
        ...
        if isinstance(payload, dict):
            detail = payload
            # §error-handling-P1: Parse structured ErrorResponse from gateway
            # Format: {"code":"NAMESPACE_DENIED","message":"...","suggestion":"..."}
