...
def hitl_gate(
    ...
    """
    Gate function for high-risk actions that require human approval.

    Call this before executing any tool or action that is considered risky.
    Raises ``PermissionError`` if the human reviewer denies the request;
    returns the ``ApprovalResult`` typed-dict on approval so the caller
    can record the approval metadata.
    """
