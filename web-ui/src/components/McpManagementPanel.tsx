  ...
  return (
    <ScrollArea className="flex-1">
      <div className="mx-auto max-w-7xl space-y-3 p-3 pb-20 sm:p-4 md:pb-0">
        {/* ── Header ── */}
        <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-border/60 bg-gradient-to-br from-background/95 to-muted/25 px-3 py-2">
          <div className="flex flex-wrap items-center gap-2">
            <Plug className="h-3.5 w-3.5 text-foreground/60" />
            <h1 className="text-sm font-semibold text-foreground">MCP Server Registry</h1>
            <Badge variant="outline" className="border-border/60 bg-background/80 text-[10px]">{namespace}</Badge>
            <Badge variant="outline" className="border-orange-500/25 bg-orange-500/10 text-orange-500 text-[10px]">Saved connections drive agent attachments</Badge>
          </div>
          <Button variant="outline" size="sm" onClick={() => void loadData()} disabled={loading} className="h-6 gap-1 bg-background/80 text-[10px]">
        ...
        {stats && (
          <div className="grid grid-cols-4 gap-1.5">
            <div className="flex items-center justify-between rounded-lg border border-border/60 bg-background/80 px-2.5 py-1 shadow-sm">
              <span className="text-[10px] text-muted-foreground">Servers</span>
              <span className="text-sm font-bold tabular-nums text-foreground">{stats.total_servers}</span>
            </div>
            <div className="flex items-center justify-between rounded-lg border border-border/60 bg-background/80 px-2.5 py-1 shadow-sm">
              <span className="text-[10px] text-muted-foreground">Tools</span>
              <span className="text-sm font-bold tabular-nums text-foreground">{stats.total_tools}</span>
            </div>
            <div className="flex items-center justify-between rounded-lg border border-amber-500/25 bg-amber-500/5 px-2.5 py-1 shadow-sm">
              <span className="text-[10px] text-muted-foreground">Profiles</span>
              <span className="text-sm font-bold tabular-nums text-foreground">{stats.total_profiles}</span>
            </div>
            <div className="flex items-center justify-between rounded-lg border border-border/60 bg-background/80 px-2.5 py-1 shadow-sm">
              <span className="text-[10px] text-muted-foreground">Categories</span>
              <span className="text-sm font-bold tabular-nums text-foreground">{stats.categories}</span>
            </div>
          </div>
        )}
        ...
        <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as McpManagementTab)} className="space-y-3">
          <TabsList className="h-auto flex-wrap justify-start gap-1 rounded-xl border border-border/60 bg-background/70 p-1">
          ...
          {/* ── Registry Tab ── */}
          <TabsContent value="registry" className="animate-fade-in space-y-3">
            <Card className="border-border/60 bg-background/80 shadow-sm shadow-black/5">
              <CardContent className="space-y-2 p-3">
                ...
                <div className="flex flex-wrap items-center gap-2 text-[10px] text-muted-foreground">
                  <span className="flex items-center gap-1"><span className="h-1.5 w-1.5 rounded-full bg-sky-500" />Remote</span>
                  <span className="flex items-center gap-1"><span className="h-1.5 w-1.5 rounded-full bg-indigo-500" />Hub</span>
                  <span className="flex items-center gap-1"><span className="h-1.5 w-1.5 rounded-full bg-amber-500" />Sidecar</span>
                </div>
              </CardContent>
            </Card>
            ...
            {filteredServers.length === 0 ? (
              ...
              <div className="space-y-2">
                {readyServers.length > 0 ? (
                  <section className="space-y-1.5">
                    <div className="flex items-center justify-between">
                      <p className="text-xs font-medium text-foreground">Ready now</p>
                      <span className="text-[10px] text-muted-foreground">{readyServers.length} server{readyServers.length === 1 ? "" : "s"}</span>
                    </div>
                    <div className="grid gap-2 md:grid-cols-2 lg:grid-cols-3">
                ...
                {deferredServers.length > 0 ? (
                  <section className="space-y-1.5">
                    <p className="text-xs font-medium text-foreground">Needs setup</p>
                    <div className="grid gap-2 md:grid-cols-2 lg:grid-cols-3">
            ...
            <TabsContent value="connections" className="animate-fade-in space-y-3">
              <div className="grid gap-2 xl:grid-cols-[0.9fr_1.1fr]">
                  ...
                  <CardHeader className="pb-2">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        ...
                        <CardDescription className="text-xs">Reusable namespace-scoped bindings for remote endpoints, hub services, and managed sidecar defaults in {namespace}.</CardDescription>
                      </div>
                      <Button variant="outline" size="sm" onClick={() => handleStartNewConnection()} className="h-7 text-[11px]">
                        New connection
                      </Button>
                    </div>
                  </CardHeader>
                  <CardContent className="space-y-2">
                    <div className="rounded-xl border border-border/60 bg-muted/15 px-3 py-2 text-xs leading-4 text-muted-foreground shadow-sm shadow-black/5">
                ...
                <Card className="border-border/60 bg-background/80 shadow-sm shadow-black/5">
                  <CardHeader className="pb-2">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        ...
                        <CardDescription className="text-xs">Confirm the endpoint, store credentials once, and reuse the saved connection from agents instead of copying raw MCP strings around.</CardDescription>
                      </div>
                      {selectedConnection ? (
                        <Badge variant="outline" className={`text-[10px] ${SUPPORT_STYLES[selectedConnection.support_level]}`}>
                  ...
                  <CardContent className="space-y-3">
                    ...
                    {selectedConnectionServer ? (
                      <div className="rounded-xl border border-border/60 bg-muted/15 p-2.5 text-xs text-muted-foreground shadow-sm shadow-black/5">
                        <div className="flex flex-wrap items-center gap-2">
                          <Badge variant="outline" className={`text-[10px] ${TRANSPORT_STYLES[selectedConnectionServer.transport].border} ${TRANSPORT_STYLES[selectedConnectionServer.transport].bg} ${TRANSPORT_STYLES[selectedConnectionServer.transport].color}`}>
                            {selectedConnectionServer.transport}
                          </Badge>
                          <Badge variant="outline" className={`text-[10px] ${SUPPORT_STYLES[selectedConnectionServer.support_level]}`}>
                            {formatSupportLabel(selectedConnectionServer.support_level)}
                          </Badge>
                          <span className="leading-4">{selectedConnectionServer.description}</span>
                        </div>
                        {selectedConnectionServer.status_reason ? (
                          <p className="mt-1.5 leading-4">{selectedConnectionServer.status_reason}</p>
                        ) : null}
                    ...
                    {connectionFields.length > 0 ? (
                      ...
                      <div className="rounded-xl border border-dashed border-border/70 bg-background/40 px-3 py-3 text-sm text-muted-foreground">
                        This connection does not require extra fields. Save it directly or validate it now.
                      </div>
                    ) : null}

                    <div className="flex flex-wrap gap-2">
                      <Button size="sm" onClick={() => void handleSaveConnection(false)} disabled={connectionBusy || oauthBusy || !connectionDraft.serverId.trim()}>
                        Save connection
                      </Button>
                      <Button size="sm" variant="outline" onClick={() => void handleSaveConnection(true)} disabled={connectionBusy || oauthBusy || !connectionDraft.serverId.trim()}>
                        Save + validate
                      </Button>
                      <Button size="sm" variant="outline" onClick={() => void handleValidateSelectedConnection()} disabled={connectionBusy || oauthBusy || !selectedConnectionId}>
                        Validate now
                      </Button>
                      <Button size="sm" variant="outline" onClick={() => handleStartNewConnection()} disabled={connectionBusy || oauthBusy}>
                        Reset draft
                      </Button>
                      <Button size="sm" variant="destructive" onClick={() => void handleDeleteSelectedConnection()} disabled={connectionBusy || oauthBusy || !selectedConnectionId}>
                        Delete
                      </Button>
                    </div>

                    {selectedConnection ? (
                      <div className="rounded-xl border border-border/60 bg-muted/15 p-2.5 shadow-sm shadow-black/5">
                        ...
                        {selectedConnection.validation.message ? (
                          <p className="mt-2 text-xs text-muted-foreground">{selectedConnection.validation.message}</p>
                        ) : null}
                      </div>
                    ) : null}
                  </CardContent>
              ...
              <Card className="border-border/60 bg-background/80 shadow-sm shadow-black/5">
                <CardHeader className="pb-2">
                  ...
                  <CardDescription className="text-xs">See which agents currently depend on the selected saved connection.</CardDescription>
                </CardHeader>
                <CardContent>
                  {!selectedConnection ? (
                    ...
                    <div className="flex flex-wrap gap-2">
                      {connectionBindings.map((binding) => (
                        <Badge key={`${binding.namespace}-${binding.agent_name}`} variant="outline" className="px-2 py-0.5 text-xs">
          ...
          {/* ── Profiles Tab ── */}
          <TabsContent value="profiles" className="animate-fade-in space-y-3">
            <Card className="border-border/60 bg-background/80 shadow-sm shadow-black/5">
          ...
          {/* ── Architecture Tab ── */}
          <TabsContent value="architecture" className="animate-fade-in space-y-3">
            <div className="grid gap-3 lg:grid-cols-2">
              <Card className="border-border/60 bg-background/80 shadow-sm shadow-black/5">
                <CardHeader className="pb-2">
                  <div className="flex items-center gap-2">
                    <div className="flex h-7 w-7 items-center justify-center rounded-lg border border-sky-500/25 bg-sky-500/10 text-sky-500">
                      <Globe className="h-4 w-4" />
                    </div>
                    <div>
                      <CardTitle className="text-sm">Remote MCP Servers</CardTitle>
                      <CardDescription className="text-xs">Zero container overhead</CardDescription>
                    </div>
                  </div>
                </CardHeader>
                <CardContent className="space-y-2 text-sm text-muted-foreground">
                  ...
                  <div className="rounded-lg border border-border/60 bg-background/60 p-2 font-mono text-xs">
              ...
              <Card className="border-border/60 bg-background/80 shadow-sm shadow-black/5">
                <CardHeader className="pb-2">
                  <div className="flex items-center gap-2">
                    <div className="flex h-7 w-7 items-center justify-center rounded-lg border border-violet-500/25 bg-violet-500/10 text-violet-500">
                      <Server className="h-4 w-4" />
                    </div>
                    <div>
                      <CardTitle className="text-sm">Hub Servers</CardTitle>
                      <CardDescription className="text-xs">Shared across all agents</CardDescription>
                    </div>
                  </div>
                </CardHeader>
                <CardContent className="space-y-2 text-sm text-muted-foreground">
              ...
              <Card className="border-border/60 bg-background/80 shadow-sm shadow-black/5">
                <CardHeader className="pb-2">
                  <div className="flex items-center gap-2">
                    <div className="flex h-7 w-7 items-center justify-center rounded-lg border border-amber-500/25 bg-amber-500/10 text-amber-500">
                      <Box className="h-4 w-4" />
                    </div>
                    <div>
                      <CardTitle className="text-sm">Sidecar Containers</CardTitle>
                      <CardDescription className="text-xs">Per-agent pod isolation</CardDescription>
                    </div>
                  </div>
                </CardHeader>
                <CardContent className="space-y-2 text-sm text-muted-foreground">
              ...
              <Card className="border-border/60 bg-background/80 shadow-sm shadow-black/5">
                <CardHeader className="pb-2">
                  <div className="flex items-center gap-2">
                    <div className="flex h-7 w-7 items-center justify-center rounded-lg border border-emerald-500/25 bg-emerald-500/10 text-emerald-500">
                      <Shield className="h-4 w-4" />
                    </div>
                    <div>
                      <CardTitle className="text-sm">Security Model</CardTitle>
                      <CardDescription className="text-xs">Per-agent NetworkPolicy enforcement</CardDescription>
                    </div>
                  </div>
                </CardHeader>
                <CardContent className="space-y-2 text-sm text-muted-foreground">
...
function McpOAuthSessionCard({
  ...
  return (
    <div className="rounded-xl border border-border/60 bg-background/75 p-3 shadow-sm shadow-black/5">
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div className="flex min-w-0 items-start gap-2">
          <div className="flex h-7 w-7 items-center justify-center rounded-lg border border-border/60 bg-background/80 text-foreground/80">
      ...
      {oauth?.scope.length ? (
        <div className="mt-2 flex flex-wrap gap-1.5">
          {oauth.scope.map((scope) => (
            <Badge key={scope} variant="outline" className="rounded-full px-2 py-0.5 text-[10px] border-border/60 text-muted-foreground">
              {formatOauthScopeLabel(scope)}
            </Badge>
          ))}
        </div>
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-1.5">
              <h3 className="font-medium text-xs text-foreground truncate">{server.name}</h3>
              <Badge variant="outline" className={`text-[9px] px-1 py-0 ${transport.bg} ${transport.color} ${transport.border}`}>
                {transport.label}
              </Badge>
              <Badge variant="outline" className={`text-[9px] px-1 py-0 ${SUPPORT_STYLES[server.support_level]}`}>
                {formatSupportLabel(server.support_level)}
              </Badge>
            </div>
            <p className="mt-0.5 text-[11px] leading-tight text-muted-foreground line-clamp-1">{server.description}</p>
          </div>
        </div>

        <div className="flex items-center justify-between gap-2">
          <div className="flex flex-wrap gap-1">
            <Badge variant="outline" className={`text-[9px] px-1.5 py-0 ${categoryStyle}`}>{server.category}</Badge>
            {server.auth_type !== "none" && (
              <Badge variant="outline" className="text-[9px] px-1.5 py-0 border-border/60">{server.auth_type}</Badge>
            )}
            {server.tags.slice(0, 2).map((tag) => (
              <span key={tag} className="rounded-full bg-muted/60 px-1.5 py-0 text-[9px] text-muted-foreground">{tag}</span>
            ))}
          </div>
          <span className="flex items-center gap-1 text-[10px] text-muted-foreground whitespace-nowrap">
            <Wrench className="h-2.5 w-2.5" />{server.tools_count}
          </span>
        </div>
      </CardContent>
    </Card>
    ...
    <Card className="animate-fade-in border-border/60 bg-background/90 shadow-xl shadow-black/10">
      <CardHeader className="pb-2">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            {server.attachable ? (
              <Button size="sm" onClick={() => onCreateConnection(server.id)} className="text-xs h-7">
      ...
      <CardContent className="space-y-3">
        <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
          <div className="rounded-lg border border-border/60 bg-background/60 p-2.5">
            ...
            <p className={`mt-0.5 text-sm font-medium ${transport.color}`}>{transport.label}</p>
          </div>
          <div className="rounded-lg border border-border/60 bg-background/60 p-2.5">
            <p className="text-[10px] uppercase tracking-[0.16em] text-muted-foreground/70">Protocol</p>
            <p className="mt-0.5 text-sm font-medium text-foreground">{getProtocolLabel(server)}</p>
          </div>
          <div className="rounded-lg border border-border/60 bg-background/60 p-2.5">
            <p className="text-[10px] uppercase tracking-[0.16em] text-muted-foreground/70">Readiness</p>
            <Badge variant="outline" className={`mt-1.5 text-[10px] ${SUPPORT_STYLES[server.support_level]}`}>
              {formatSupportLabel(server.support_level)}
            </Badge>
          </div>
          <div className="rounded-lg border border-border/60 bg-background/60 p-2.5">
            <p className="text-[10px] uppercase tracking-[0.16em] text-muted-foreground/70">Authentication</p>
            <p className="mt-0.5 text-sm font-medium text-foreground">{formatAuthLabel(server.auth_type)}</p>
          </div>
          <div className="rounded-lg border border-border/60 bg-background/60 p-2.5">
            <p className="text-[10px] uppercase tracking-[0.16em] text-muted-foreground/70">Tools</p>
            <p className="mt-0.5 text-sm font-medium text-foreground">{server.tools_count}</p>
          </div>
          <div className="rounded-lg border border-border/60 bg-background/60 p-2.5">
            <p className="text-[10px] uppercase tracking-[0.16em] text-muted-foreground/70">Category</p>
            <p className="mt-0.5 text-sm font-medium text-foreground capitalize">{server.category.replace("-", " ")}</p>
          </div>
        </div>
        ...
        <div className="rounded-lg border border-border/60 bg-background/60 p-2.5">
          <div className="flex flex-wrap items-start justify-between gap-2">
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-0.5 text-sm font-medium text-foreground">{getDeploymentModelLabel(server)}</p>
              {server.connection_notes ? (
                <p className="mt-0.5 text-xs leading-4 text-muted-foreground">{server.connection_notes}</p>
              ) : null}
            </div>
        ...
        {authPreview ? (
          <div className="rounded-lg border border-border/60 bg-background/60 p-2.5">
            ...
            <p className="mt-1.5 text-xs leading-4 text-muted-foreground">{authPreview.help}</p>
          </div>
        ) : null}
        ...
        {server.status_reason && (
          <div className="rounded-lg border border-border/60 bg-background/60 p-2.5">
            ...
            <p className="text-xs leading-4 text-muted-foreground">{server.status_reason}</p>
          </div>
        )}
            ...
            <ScrollArea className="mt-2 max-h-32 pr-3">
              <div className="flex flex-wrap gap-1">
                {server.tool_names.map((toolName) => (
                  <Badge key={toolName} variant="secondary" className="rounded-full px-2 py-0 text-[10px]">
                    {toolName}
                  </Badge>
                ))}
              </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              {server.config_schema.map((field) => (
                <div key={field.key} className="rounded-lg border border-border/60 bg-background/60 p-2.5">
                  <div className="flex items-center gap-2">
        ...
        <div className="flex flex-wrap gap-1">
          {server.tags.map((tag) => (
            <Badge key={tag} variant="secondary" className="rounded-full px-2 py-0 text-[10px]">{tag}</Badge>
          ))}
        </div>
  ...
  return (
    <Card className={`${colors.border} ${colors.bg} border bg-background/80 shadow-sm shadow-black/5 transition-all hover:-translate-y-0.5 hover:shadow-lg`}>
      <CardContent className="p-3 space-y-2.5">
        <div className="flex items-start gap-2">
          <div className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg border ${colors.border} bg-background/80`}>
            <Icon className={`h-4 w-4 ${colors.accent}`} />
          </div>
          <div className="min-w-0 flex-1">
            <h3 className="font-semibold text-sm text-foreground">{profile.name}</h3>
            <p className="mt-0.5 text-xs leading-4 text-muted-foreground">{profile.description}</p>
          </div>
        </div>

        <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
          <div className="rounded-lg border border-border/40 bg-background/60 p-1.5 text-center">
            <p className="text-base font-bold tabular-nums leading-none text-foreground">{profile.resolved_servers.length}</p>
            <p className="text-[10px] text-muted-foreground">Servers</p>
          </div>
          <div className="rounded-lg border border-border/40 bg-background/60 p-1.5 text-center">
            <p className="text-base font-bold tabular-nums leading-none text-foreground">{profile.total_tools}</p>
            <p className="text-[10px] text-muted-foreground">Tools</p>
          </div>
        </div>

        <div className="flex flex-wrap gap-1">
          <Badge variant="outline" className={`text-[10px] px-1.5 py-0 ${SUPPORT_STYLES[profile.support_level]}`}>
            {profile.attachable_servers.length}/{profile.resolved_servers.length} attachable now
          </Badge>
          {profile.blocked_servers.length > 0 && (
            <Badge variant="outline" className="text-[10px] px-1.5 py-0 border-amber-500/20 bg-amber-500/5 text-amber-500">
            ...
            {profile.resolved_servers.map((s) => {
              ...
              return (
                <Badge
                  key={s.id}
                  variant="outline"
                  className={`text-[10px] px-1.5 py-0 ${tStyle.bg} ${tStyle.color} ${tStyle.border}`}
                >
                  {s.name}
                </Badge>
              );
        ...
        <div className="flex flex-wrap gap-1">
          {profile.tags.map((tag) => (
            <span key={tag} className="rounded-full bg-background/60 px-2 py-0 text-[10px] text-muted-foreground">{tag}</span>
          ))}
        </div>
      </CardContent>
    </Card>
