    ...
    <div className="space-y-8">
      ...
      <div id="mcp-sidecars">
        ...
        <DocsTable headers={["Sidecar", "Capability"]} rows={[
          ...
          ["kubernetes", "In-cluster resource queries and mutations"],
      ...
      <div id="mcp-hub">
        ...
        <DocsTable headers={["Aspect", "Detail"]} rows={[
          ...
          ["Per-server isolation", "Each MCP server gets its own Deployment + ClusterIP Service + NetworkPolicy"],
          ["Security", "Non-root UID 1000, readOnlyRootFilesystem, drop ALL capabilities, default-deny NetworkPolicies"],
          ["Auth", "Bearer token verification via HMAC constant-time comparison from K8s Secrets"],
...
function CliSection() {
  return (
    <div className="space-y-8">
      ...
      <CodeBlock code={`# ── Agent Management ──
agentctl list agents                    # List all agents
agentctl create -f agent.yaml           # Create an agent from YAML
agentctl get my-agent                   # Get agent details
agentctl edit my-agent                  # Edit agent spec in your editor
agentctl delete my-agent                # Delete an agent
agentctl clone my-agent --name clone    # Clone an agent

# ── Invocation ──
agentctl invoke my-agent --prompt "Summarize this PR"
agentctl invoke my-agent --stream --prompt "..."   # SSE streaming

# ── Workflows ──
agentctl workflow list                  # List workflows
agentctl workflow create -f wf.yaml    # Create a workflow
agentctl workflow trigger my-wf         # Trigger a workflow run
agentctl workflow cancel my-wf          # Cancel a running workflow
agentctl workflow retry my-wf           # Retry failed workflow steps
agentctl workflow logs my-wf            # Get workflow logs

# ── Policies ──
agentctl policy list                    # List policies
agentctl policy create -f policy.yaml  # Create a policy

# ── Chat & Memory ──
agentctl chat list --agent my-agent     # List chat sessions
agentctl memory list --agent my-agent   # List memory records
agentctl memory promote <id>            # Promote a memory record

# ── Export & Import ──
agentctl export agent my-agent --include-policies --include-workflows > bundle.yaml
agentctl import -f bundle.yaml          # Import a YAML bundle
...
function ExportImportSection() {
  return (
    <div className="space-y-8">
      ...
      <div id="ei-cli">
        <h3 className="text-lg font-bold text-[oklch(0.95_0.005_264)] mb-3">CLI Export</h3>
