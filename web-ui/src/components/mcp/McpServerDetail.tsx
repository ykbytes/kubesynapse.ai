...
export function McpServerDetail({ server, onClose, onCreateConnection }: McpServerDetailProps) {
  ...
  return (
    <Dialog open={!!server} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-3xl max-h-[85vh] p-0 gap-0 grid-rows-[auto_minmax(0,1fr)]">
        <DialogHeader className="shrink-0 space-y-0 border-b border-border/40 p-5">
          <div className="flex items-start justify-between gap-3">
            ...
            <Button variant="ghost" size="icon" className="size-7 shrink-0" onClick={onClose} aria-label="Close">
              <X className="size-4" />
            </Button>
          </div>

          {server.attachable && (
            <Button size="sm" onClick={() => { onCreateConnection(server.id); onClose(); }} className="mt-3 w-fit h-7 text-xs">
          ...
          <div className="space-y-5 p-5">
            ...
            {server.sidecar_image && (
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground">
                  kubesynapse provides the default sidecar build for this toolkit.
            ...
            <div className="space-y-2">
              ...
              {server.tool_names.length > 0 ? (
                ...
                <p className="text-sm text-muted-foreground">
                  Tool catalog not published in registry metadata yet.
                </p>
              )}
            </div>
                  ...
                  {server.config_schema.map((field) => (
                    <div key={field.key} className="rounded-lg border border-border/40 bg-muted/20 p-3">
                      <div className="flex items-center gap-2">
                        ...
                        {field.required && (
                          <Badge variant="destructive" className="px-1.5 py-0 text-xs">Required</Badge>
                        )}
                        {field.is_credential && (
                          <Badge variant="outline" className="px-1.5 py-0 text-xs">Secret</Badge>
                        )}
                      </div>
                      <p className="mt-1 text-xs text-muted-foreground">
                        {field.type} · {field.group ?? "general"}
                      </p>
                    </div>
                  ))}
                </div>
            ...
            <div className="text-sm leading-relaxed text-muted-foreground">
              <p>
                ...
                {server.attachable && server.transport === "hub" &&
                  "This server is attachable through the shared mcp-hub namespace. Create a saved connection for it, then attach that connection to an agent."}
                {server.attachable && server.transport === "sidecar" &&
                  "This server will be deployed as a container in the agent's pod. Create a saved connection for it, then attach to the agent so the sidecar launches automatically."}
                {server.attachable && server.transport === "remote" &&
                  (server.endpoint
                    ? "This server is modeled as a direct remote MCP endpoint. Saved connections default to the published registry URL; add credentials only when needed."
                    : "This server is modeled as a self-hosted remote MCP integration. Create a saved connection only if you operate your own deployment.")}
              </p>
            </div>
          </div>
        </ScrollArea>
