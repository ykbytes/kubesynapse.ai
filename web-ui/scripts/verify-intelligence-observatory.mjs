...
const checks = [
  ...
  ["observatory has a restored timeline-first tab contract", observatory.includes('type ObservatoryTab = "timeline" | "analytics" | "trace" | "optimise" | "logs" | "compare";')],
  ["observatory restores the ergonomic timeline view", observatory.includes("ExecutionTimelineView") && observatory.includes("DetailDrawer")],
  ["observatory adds compact analytics view", observatory.includes("AnalyticsView")],
  ...
  ["optimise prompt requests optimized manifest copies", observatory.includes("candidate_manifest_bundle fenced YAML for copied workflow/agent resources")],
  ["optimise tab documents least privilege deploy agent", observatory.includes("deployment_access_model") && observatory.includes("least-privilege RBAC")],
  ...
  ["timeline is the default run workspace", observatory.includes('useState<ObservatoryTab>("timeline")') && observatory.includes('setActiveTab("timeline")')],
