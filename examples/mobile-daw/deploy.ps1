# deploy.ps1 - Deploy BeatForge Mobile DAW workflow
# Usage: .\deploy.ps1
...
Write-Host '=== Deploying BeatForge Mobile DAW v4 - skills auto-loaded ===' -ForegroundColor Cyan

# 0. Clean up any existing resources
Write-Host "`n[0/3] Cleaning up old resources..." -ForegroundColor Yellow
$ErrorActionPreference = "Continue"
kubectl delete agentworkflow mobile-daw -n $ns 2>&1 | Out-Null
kubectl delete aiagent daw-agent -n $ns 2>&1 | Out-Null
$ErrorActionPreference = "Stop"
...
Write-Host "`n[3/3] Applying workflow (starts 4-step execution)..." -ForegroundColor Yellow
kubectl apply -f "$dir\workflow.yaml" -n $ns

Write-Host "`n=== Deployment complete ===" -ForegroundColor Green
Write-Host 'Steps: tooling 5t -> foundation 10t -> implement 14t -> polish 14t' -ForegroundColor Cyan
Write-Host 'Skills: auto-loaded - known-pitfalls, audio-engine, integration-wiring, strict-types, react-native-quality, verification-gates' -ForegroundColor Cyan
Write-Host 'Monitor at: http://localhost:18181' -ForegroundColor Cyan
